...
Setting `volatile: true` on the cursors path tells Lark that this will be frequently-written data that doesn't need to be persisted to disk. This is one of the powerful features of Lark and allows you to use the same server for both your cursor presence fire-and-forget data alongside your persisted data. If you don't set this flag, all of your cursor positions are written to disk, which can add up fast, so don't forget it!
...
- **Open the admin Database Editor** in a third tab while the app runs — you can watch the JSON tree mutate as notes are placed and dragged.
